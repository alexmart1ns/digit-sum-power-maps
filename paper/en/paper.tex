...
The same Gaussian model, pushed to $4 \le D \le 300$ via \texttt{scripts/sweep\_label.py} (\texttt{data/split/label\_sweep\_latest.md}), does not flatten: decade amplitude on $10 \le D \le 99$ is $0.113$ (full $\{\log_{10} D\}$ period), on $100 \le D \le 300$ is $0.115$, and cross-decade phase correlation is Pearson $r=0.735$ ($n=201$ overlapping $\{\log_{10} D\}$ points). This is \textbf{model output}, not Monte Carlo measurement.
...
A Gaussian-window diagnostic (\texttt{scripts/sweep\_label.py}; \texttt{data/split/label\_sweep\_latest.md}) evaluates the \S\ref{sec:split} model on signature $\{0\}$ for $4 \le D \le 300$ without sampling $n$ (ceiling $V=4789$). Verdict: \textbf{SURVIVES} --- decade amplitude remains $\approx 0.11$--$0.12$, cross-decade phase correlation is Pearson $r=0.735$, and the curves remain in exact antiphase; under Hypothesis LLT, Conjecture~\ref{con:splitpilot} follows.
\end{conjecture}
