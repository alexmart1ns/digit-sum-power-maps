...
$\#\Per(k,m)$ depends on $k$ only through $\rad(k)$, the product of the distinct primes dividing $k$; in particular its $2$-part collapses entirely at the single transition $2 \nmid k \to 2 \mid k$ and is unchanged for all higher $2$-adic valuations $v_2(k)$.
\end{corollary}
...
\]
as $D\to\infty$, and the tails $\mathbb{P}(\lvert S_b(n^k)-\mu_D\rvert>C\sigma_D)$ vanish as $C\to\infty$ uniformly in $D$. (Casting out nines forces $S_b(n^k)\equiv n^k\pmod m$ exactly.) This is the form in which a restriction of the local limit theorem of Drmota--Mauduit--Rivat \cite{dmr} to a $b$-adic band and an arithmetic progression would be used. It is an input, not a claim of the present paper.
