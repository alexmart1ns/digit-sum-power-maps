...
The same Gaussian model, pushed to $4 \le D \le 300$ via `scripts/sweep_label.py` (`data/split/label_sweep_latest.md`), does not flatten: decade amplitude on $10 \le D \le 99$ is $0.113$ (full $\{\log_{10} D\}$ period), on $100 \le D \le 300$ is $0.115$, and cross-decade phase correlation is Pearson $r=0.735$ ($n=201$ overlapping $\{\log_{10} D\}$ points between decades $10^1$ and $10^2$). The curves remain in exact antiphase (Pearson $-1$, algebraic). This is **model output**, not Monte Carlo measurement.
...
This is recorded as an observation rather than a result: non-existence of a limit is a statement about all $D$, and finitely many bands cannot establish it. **Monte Carlo** (`split_scale.py`, $4 \le D \le 90$, $60{,}000$ samples per band): the individual masses have not settled by $D=90$, with amplitude $\approx 0.18$ on signature $\{0\}$, two orders of magnitude above the sampling noise $\sigma \approx 0.002$. **Gaussian model** (`sweep_label.py`, $4 \le D \le 300$, no sampling of $n$): persistent oscillation with amplitude $\approx 0.15$ and cross-decade phase correlation $0.735$, consistent with Conjecture 10.6$'$.
...
A Gaussian-window diagnostic (`scripts/sweep_label.py`; `data/split/label_sweep_latest.md`) evaluates the §7.5 model on signature $\{0\}$ for $4 \le D \le 300$ without sampling $n$ (ceiling $V=4789$). On the primary attractor $\{27\}$, decade amplitude remains $\approx 0.11$–$0.12$ ($0.113$ on $10 \le D \le 99$, full $\{\log_{10} D\}$ period; $0.115$ on $100 \le D \le 300$). Cross-decade phase correlation is Pearson $r=0.735$ ($n=201$ overlapping $\{\log_{10} D\}$ points between decades $10^1$ and $10^2$). The curves $\delta_{18}(D)$ and $\delta_{27}(D)$ remain in exact antiphase (Pearson $-1$, algebraic), always summing to $1/3$. Verdict: **SURVIVES** — under Hypothesis LLT, Conjecture 10.6$'$ follows.
