...
O mesmo modelo gaussiano, estendido a $4 \le D \le 300$ via \texttt{scripts/sweep\_label.py} (\texttt{data/split/label\_sweep\_latest.md}), não se achata: amplitude por década em $10 \le D \le 99$ é $0{,}113$, em $100 \le D \le 300$ é $0{,}115$, e a correlação de fase entre décadas é Pearson $r=0{,}735$. Isto é \textbf{saída do modelo}, não medição Monte Carlo.
...
Um diagnóstico em janela gaussiana (\texttt{scripts/sweep\_label.py}; \texttt{data/split/label\_sweep\_latest.md}) avalia o modelo da \S\ref{sec:split} na assinatura $\{0\}$ para $4 \le D \le 300$ sem amostragem de $n$. Veredito: \textbf{SURVIVES} --- amplitude por década permanece $\approx 0{,}11$--$0{,}12$, correlação de fase entre décadas é Pearson $r=0{,}735$; sob a Hipótese LLT, segue a Conjectura~\ref{con:splitpilot}.
\end{conjecture}
