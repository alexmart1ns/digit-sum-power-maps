...
A gaussiana da \S\ref{sec:split} é o termo principal do LLT. Comparar o MAE de uma inversão de Fourier truncada com a gaussiana não prova a conjectura (as duas já coincidem até o ruído de amostragem). O trabalho é: (i) a Hipótese LM para a rotulação, diagnosticada pelas médias $\Psi_j(V)$ no espaço $v$; (ii) se \cite{delange,drmota-poly,drmota-grabner} implicam LM; (iii) o input LLT.
