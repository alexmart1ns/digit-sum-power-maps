...
O mesmo modelo gaussiano, estendido a $4 \le D \le 300$ via `scripts/sweep_label.py` (`data/split/label_sweep_latest.md`), não se achata: amplitude por década em $10 \le D \le 99$ é $0{,}113$ (período completo de $\{\log_{10} D\}$), em $100 \le D \le 300$ é $0{,}115$, e a correlação de fase entre décadas é Pearson $r=0{,}735$ ($n=201$ pontos sobrepostos de $\{\log_{10} D\}$ entre as décadas $10^1$ e $10^2$). As curvas permanecem em antifase exata (Pearson $-1$, algébrico). Isto é **saída do modelo**, não medição Monte Carlo.
...
Isto é registrado como observação e não como resultado: a não existência de um limite é uma afirmação sobre todo $D$, e um número finito de faixas não a estabelece. **Monte Carlo** (`split_scale.py`, $4 \le D \le 90$, $60{,}000$ amostras por faixa): as massas individuais não se assentaram até $D=90$, com amplitude $\approx 0{,}18$ na assinatura $\{0\}$, duas ordens de magnitude acima do ruído $\sigma \approx 0{,}002$. **Modelo gaussiano** (`sweep_label.py`, $4 \le D \le 300$, sem amostragem de $n$): oscilação persistente com amplitude $\approx 0{,}15$ e correlação de fase entre décadas $0{,}735$, coerente com a Conjectura 10.6$'$.
...
Um diagnóstico em janela gaussiana (`scripts/sweep_label.py`; `data/split/label_sweep_latest.md`) avalia o modelo da §7.5 na assinatura $\{0\}$ para $4 \le D \le 300$ sem amostragem de $n$ (teto $V=4789$). No atrator primário $\{27\}$, a amplitude por década permanece $\approx 0{,}11$–$0{,}12$ ($0{,}113$ em $10 \le D \le 99$, período completo de $\{\log_{10} D\}$; $0{,}115$ em $100 \le D \le 300$). A correlação de fase entre décadas é Pearson $r=0{,}735$ ($n=201$ pontos sobrepostos de $\{\log_{10} D\}$ entre as décadas $10^1$ e $10^2$). As curvas $\delta_{18}(D)$ e $\delta_{27}(D)$ permanecem em antifase exata (Pearson $-1$, algébrico), sempre somando $1/3$. Veredito: **SURVIVES** — sob a Hipótese LLT, segue a Conjectura 10.6$'$.
