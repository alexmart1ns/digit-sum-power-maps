...
Medição de Monte Carlo da divisão restrita a $n$ com exatamente $D$ dígitos em base $b$ (`scripts/split_scale.py`, $(k,b) = (3,10)$, $4 \le D \le 90$, $60{,}000$ amostras por faixa, amostragem $\sigma \approx 0{,}002$) fornece o seguinte quadro nítido e reproduzível:
...
**Problema 10.2 (Caracterização da justeza).** Para quais pares $(k,b)$ vale a igualdade $|C(k,b)| = \mathrm{Cyc}(\varphi_{k,b-1})$? Os dados sugerem $b$ pequeno (especialmente $b = 2,3$) ou dinâmica modular trivial; a frequência de igualdade exata é ela própria sensível à faixa (§8.3).
...
**Problema 10.4 (Estrutura do grafo de predecessores).** Para $(k,b)$ fixo e $A \in C(k,b)$, a árvore de bacia enraizada em $A$: qual é sua distribuição de graus? Segue uma lei de potência?

**Problema 10.5 (Cota superior).** Provar uma cota superior complementar para $|C(k,b)|$; um candidato natural é o limiar de contração $N^*(k,b)$ do Lema 3.2.

**Problema 10.6 (Teoria rigorosa da divisão).** Substituir a gaussiana numérica da §7.5 pelo termo de Fourier exato de Mellin–Perron para $S_b(n^k)$ e provar rigorosamente a oscilação quase-periódica da divisão por dígito; determinar se o enunciado já está implícito em [13–15].
